...
const passwordDatabase = fs.readFileSync(path.join(__dirname, './day2.txt'), 'utf8')
const passwords = passwordDatabase.split('\n')

const passwordPhilosophy = (passwords) => {
  let validPasswordCount = 0;

  for(let key in passwords){
    let policy = key.split('')
    ...
    let passwordArray = passwords[key].split('')

    for(let i = 0; i < passwordArray.length; i++){
      if(passwordArray[i] === letter) letterCount++
    }

    if(letterCount >= low && letterCount <= high) validPasswordCount++
  }
